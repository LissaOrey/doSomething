...
export const goUp = (copy, arr, setScore, score ) => {
    copy.forEach(el => {
        for (let i = 0; i < arr.length; i++) {
            ...
            // true?1:0 такое выражение не работает
            if (el.x === 3 && el.y === i) {
                el.value = a
            } else if (el.x === 2 && el.y === i) {
                el.value = b
            } else if (el.x === 1 && el.y === i) {
                el.value = c
            } else if (el.x === 0 && el.y === i) {
                el.value = d
            }
        }
    })
...
export const goRight = (copy, arr, setScore, score) => {
    copy.forEach(el => {
        for (let i = 0; i < arr.length; i++) {
            ...
            if (el.x === i && el.y === 0) {
                el.value = a
            } else if (el.x === i && el.y === 1) {
                el.value = b
            } else if (el.x === i && el.y === 2) {
                el.value = c
            } else if (el.x === i && el.y === 3) {
                el.value = d
            }
        }
    })
